seq_inference for TensorFlow Lite inspired by TensorFlow's label_image.
...
> bazel build --config monolithic --cxxopt=-std=c++11 \
  ...
  --cpu=arm64-v8a \
  //tensorflow/lite/examples/seq_inference:seq_inference
```
or
```
...
```
> bazel build --config opt --cxxopt=-std=c++11 //tensorflow/lite/examples/seq_inference:seq_inference
```
