...
#include "tensorflow/contrib/lite/examples/seq_inference/get_top_n.h"
...
void RunInference(Settings* s) {
  ...
  switch (interpreter->tensor(input)->type) {
    case kTfLiteFloat32:
      ...
      feed<float>(interpreter->typed_tensor<float>(input), in.data(), wanted_height, wanted_width, s);
      break;
    case kTfLiteUInt8:
      feed<uint8_t>(interpreter->typed_tensor<uint8_t>(input), in.data(), wanted_height, wanted_width, s);
      break;
    default:
...
int main(int argc, char** argv) {
  return tflite::seq_inference::Main(argc, argv);
}
